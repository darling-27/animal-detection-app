...
const Color _kDangerRed = Color(0xFFFF1744);
const Color _kSafeGreen = Color(0xFF00E676);

class DetectionMapScreen extends StatefulWidget {
  final HistoryData alert;

  const DetectionMapScreen({super.key, required this.alert});

  @override
  State<DetectionMapScreen> createState() => _DetectionMapScreenState();
}

class _DetectionMapScreenState extends State<DetectionMapScreen>
  ...
  // ─────────────────────────────────────
  // THEME COLOR BASED ON DANGER STATUS
  // ─────────────────────────────────────
  Color get _themeColor =>
      widget.alert.isDangerous ? _kDangerRed : _kSafeGreen;
  ...
  Future<void> _fetchUserLocation() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        Future.delayed(const Duration(milliseconds: 300), _fitBothMarkers);
      }
    } catch (e) {
  ...
  Future<void> _openInGoogleMaps() async {
    ...
    try {
      ...
      final List<Map<String, String>> urlOptions = [
        ...
        {
          'url':
          'https://www.google.com/maps/dir/?api=1&origin=$userLat,$userLng&destination=$animalLat,$animalLng&travelmode=driving',
          'scheme': 'https'
        },
        {
          'url':
          'https://www.google.com/maps/dir/$userLat,$userLng/$animalLat,$animalLng',
          'scheme': 'https'
        },
        {
          'url':
          'http://maps.apple.com/?saddr=$userLat,$userLng&daddr=$animalLat,$animalLng&dirflg=d',
          'scheme': 'http'
        },
      ];
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Column(
            ...
            children: [
              const Text('Unable to open maps',
              ...
              children: [
                ...
                Expanded(
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        Icon(Icons.pets, color: _themeColor, size: 20),
                        const SizedBox(width: 8),
                        Flexible(
                          child: Text(
                            '${widget.alert.animalType[0].toUpperCase()}${widget.alert.animalType.substring(1)} Detected',
                        ...
                        const SizedBox(width: 8),
                        if (widget.alert.isDangerous)
                          Container(
                            padding: const EdgeInsets.symmetric(
                                horizontal: 8, vertical: 3),
                            decoration: BoxDecoration(
                              color: _kDangerRed.withValues(alpha: 0.2),
                          ...
                          Container(
                            padding: const EdgeInsets.symmetric(
                                horizontal: 8, vertical: 3),
                            decoration: BoxDecoration(
                              color: _kSafeGreen.withValues(alpha: 0.2),
                              borderRadius: BorderRadius.circular(6),
                            ),
                            child: const Text(
                              'SAFE',
        ...
        CircleLayer(
          circles: [
            CircleMarker(
              ...
              borderColor: _themeColor.withValues(alpha: 0.4),
              borderStrokeWidth: 2,
            ...
            CircleMarker(
              ...
              borderColor: _themeColor.withValues(alpha: 0.2),
              borderStrokeWidth: 1,
              useRadiusInMeter: true,
            ),
          ],
  ...
  Widget _buildBottomSheet() {
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        border: Border(
          top: BorderSide(
              color: _themeColor.withValues(alpha: 0.3), width: 2),
        ),
      ),
      child: SafeArea(
        top: false,
                      ...
                      children: [
                        Row(
                          children: [
                            Flexible(
                              child: Text(
                                '${alert.animalType[0].toUpperCase()}${alert.animalType.substring(1)}',
                                style: const TextStyle(
                                  color: Colors.white,
                                  fontSize: 18,
                                  fontWeight: FontWeight.bold,
              ...
              Row(
                children: [
                  _buildStatChip(
                    ...
                    label: 'Category',
                    value:
                    '${alert.animalCategory[0].toUpperCase()}${alert.animalCategory.substring(1)}',
                    color: Colors.amber,
                  ),
                  const SizedBox(width: 10),
                  _buildStatChip(
